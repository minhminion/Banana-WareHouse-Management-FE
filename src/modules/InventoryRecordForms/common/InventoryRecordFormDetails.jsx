...
import InventoryRecordFormDetailsStatus from "./components/InventoryRecordFormDetailsStatus";
import { MODULE_NAME } from "../constants/models";
import ListInventoryRecordFormProducts from "./components/ListInventoryRecordFormProducts";
import InventoryRecordFormDetailsStatusStepper from "./components/InventoryRecordFormDetailsStatusStepper";
...
const InventoryRecordFormDetails = ({
  inventoryRecordDetails,
  initialValues,
  isEdit = true,
  onSubmit,
  ...
  const getStatusConfirmContent = (value) => {
    let content = {
      title: "Thay đổi trạng thái phiếu kiểm kho ?",
  ...
  return (
    <Box className={classes.root}>
      {values.status === ENUMS.INVENTORY_RECORD_STATUS.CANCELED && (
        <Box clone mb={2}>
       ...
       {values.id && <InventoryRecordFormDetailsStatusStepper status={values.status} />}

      <Grid container spacing={3}>
        <Grid item className={clsx(classes.root, classes.leftSide)}>
          {/*Status */}
          <InventoryRecordFormDetailsStatus
            value={values.status}
        ...
        <Grid item className={clsx(classes.root, classes.rightSide)}>
          <Box className={classes.productDescription}>
            <ListInventoryRecordFormProducts
              listProduct={inventoryRecordDetails}
              isEdit={isEdit && values.status === INVENTORY_RECORD_STATUS.NEW}
              data={values.inventoryRecordDetails}
              errors={errors}
...
export default InventoryRecordFormDetails;
