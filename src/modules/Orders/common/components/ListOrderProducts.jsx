...
const ListOrderProducts = ({ data, onChange, isEdit = false }) => {
          ...
          <CSSTransition key={product.id} timeout={500} classNames="fade">
            <TableRow>
              ...
              <TableCell align="left">
                <InputBase
                  disabled={!isEdit}
                  name={`singlePrice_product_${product.id}`}
                  error={row.singlePrice.length === 0}
                  value={formatNumberToVND(row.singlePrice)}
                  style={{
                    borderRadius: 8,
                  }}
                  classes={{
